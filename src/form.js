...
const terrainList = new ElementList(
  document.querySelector("#terrain-list"),
  editTerrainFeatureHTML,
);
const auraList = new ElementList(
  document.querySelector("#aura-list"),
  editAuraHTML,
);
/** @type { ElementList } */
const tokenList = new ElementList(
...
function updateAnchorField(newAnchor = undefined) {
  const originalValue =
    newAnchor === undefined ? auraAnchorField.value : newAnchor;
  ...
  for (let i = 0; i < terrainList.dataList.listElements.length; i++) {
    if (terrainList.dataList.listElements[i]) {
      auraAnchorField.appendChild(
        createOptionElement(terrainList.dataList.listElements[i].name),
      );
    }
  }
  for (let i = 0; i < tokenList.dataList.listElements.length; i++) {
    if (tokenList.dataList.listElements[i]) {
      auraAnchorField.appendChild(
        createOptionElement(tokenList.dataList.listElements[i].name),
      );
    }
  }

  auraAnchorField.selectedIndex = Math.max(
    0,
    Array.from(auraAnchorField.children).findIndex(
...
function link() {
  ...
  tokenList.dataList.onModify.subscribe((oldData, newData) =>
    onAnchorRename(oldData.name, newData.name),
  );
  tokenList.dataList.onRemove.subscribe((data) =>
    onAnchorRename(data.name, ""),
  );
  terrainList.dataList.onAdd.subscribe(() => updateAnchorField());
  terrainList.dataList.onModify.subscribe((oldData, newData) =>
    onAnchorRename(oldData.name, newData.name),
  );
  terrainList.dataList.onRemove.subscribe((data) =>
    onAnchorRename(data.name, ""),
  );
...
export { link, start, terrainList, auraList, tokenList };
