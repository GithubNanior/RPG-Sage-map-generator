...
function serializeData() {
  return [
    serializeMap(),
    ...terrainList.listElements
      .filter((terrain) => terrain)
      .map((terrain) => serializeTerrain(terrain)),
    ...auraList.listElements
      .filter((aura) => aura)
      .map((aura) => serializeAura(aura)),
    ...tokenList.listElements
      .filter((token) => token)
...
function load(serializedData) {
  ...
  matchChunks(chunks, /^\s*\[token\]\s*$/i).forEach((chunk) =>
    tokenList.add(deserializeToken(chunk)),
  );

  matchChunks(chunks, /^\s*\[terrain\]\s*$/i).forEach((chunk) =>
    terrainList.add(deserializeTerrain(chunk)),
  );

  matchChunks(chunks, /^\s*\[aura\]\s*$/i).forEach((chunk) =>
    auraList.add(deserializeAura(chunk)),
  );
}

function deserializeMap(lines) {
  const [gridColumns, gridRows] =
    matchLine(lines, /^grid=/i, false, 2) ?? Array(2);
  const [spawnX, spawnY] = matchLine(lines, /^spawn=/i, false, 2) ?? Array(2);

  return {
    name: matchLine(lines, /^name=/i),
    mapURL:
      matchLine(lines, /^url=/i) ?? matchLine(lines, /^https?:\/\//i, true),
    gridColumns,
    gridRows,
    spawnX,
    spawnY,
...
function deserializeTerrain(lines) {
  const [x, y] =
    matchLine(lines, /^pos(ition)?=/i, false, 2) ?? [
      +matchLine(lines, /^col=/i),
      +matchLine(lines, /^row=/i),
    ];

  const [width, height] =
    matchLine(lines, /^size=/i, false, 2) ?? [
      +matchLine(lines, /^cols=/i),
      +matchLine(lines, /^rows=/i),
    ];
